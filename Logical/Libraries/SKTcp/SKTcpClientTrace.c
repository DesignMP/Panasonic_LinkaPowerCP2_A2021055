...
void SKTcpClientTrace(struct SKTcpClientTrace* inst)
{
	...
	if( inst->Enable == 1 ) {
		...
		switch( inst->Internal.Step ) {	 	
			...
			case 4: 
				inst->Internal.TimerIddle.IN = 1;
				inst->Internal.TimerIddle.PT = 1000;	
				
				if( inst->Internal.TimerIddle.Q == 1 )
				{
					inst->Internal.TimerIddle.IN = 0 ;
			...
			case 5: 		 
				...
				inst->Internal.TCP_Ioctl.ident = inst->Internal.TCP_Open.ident;
			...
			case 18:
				...
				inst->Internal.oneskorenie.PT = 100; //ms  1000
			...
			case 19:
				...
				inst->Params.Packet.internal.length_b_int = inst->Params.Packet.internal.length_int + SKTcpClientNod(inst->Params.Packet.internal.length_int);
    
				brsitoa(inst->Params.Packet.internal.length_b_int,(UDINT)&(inst->Params.Packet.internal.length_str)); 
				...
				//inst->Params.Packet.internal.xor_str = SKTcpClientXor(inst->DataSend.send_data_b,inst->Params.Packet.internal.length_int);
				
				//SKTcpClientNod(inst->Params.Packet.internal.length_int);
				inst->Params.Packet.internal.xor_str = SKTcpClientXor(inst->DataSend.send_data_b, inst->Params.Packet.internal.length_b_int+2);
				...
				/*brsstrcat((UDINT)&(inst->DataSend.DataSend), (UDINT)&("\0"));*/
			...
			case 20:
				...
				inst->Internal.TCP_Send.datalen = brsstrlen((UDINT)&inst->DataSend.DataSend) +5;
			...
			case 21:
				...
				if ( inst->Cmd.LoggerOn == 1 ) {
				
					inst->Internal.SKTcpLogger.enable=1;
					inst->Internal.SKTcpLogger.trigger=1;
					inst->Internal.SKTcpLogger.mode=0;
					inst->Internal.SKTcpLogger.pDevice=inst->pDevice;
					inst->Internal.SKTcpLogger.pFile=inst->pFile;
					inst->Internal.SKTcpLogger.pSrc=(UDINT)&inst->DataSend.DataSend;
				
					if ( inst->Internal.SKTcpLogger.active == 1 && inst->Internal.SKTcpLogger.done == 1 )
					{
						inst->Internal.SKTcpLogger.enable=0;
						inst->Internal.SKTcpLogger.trigger=0;
						inst->Internal.SKTcpLogger.mode=0;
						inst->Internal.Step = 30;
					}
				}
				else if ( inst->Cmd.LoggerOn == 0 ) {
					inst->Internal.Step = 30;
				}
			...
			case 30:
				...
				inst->Internal.TCP_Recv.ident = inst->Internal.TCP_Open.ident;
				...
				if( inst->Internal.TCP_Recv.status == 0 ) {
					...
					if( inst->Params.enaClose == 1 ) {
						...
						inst->Internal.TimerRecv.IN = 0 ;
						brsstrcat( (UDINT)&inst->Status.DataRecv, (UDINT)&("\r\n") );
						inst->Internal.Step = 31;
					}
			...
			case 31:
				...
				if ( inst->Cmd.LoggerOn == 1 ) {
					inst->Internal.SKTcpLogger.enable=1;
					inst->Internal.SKTcpLogger.trigger=1;
					inst->Internal.SKTcpLogger.mode=1;
					inst->Internal.SKTcpLogger.pDevice=inst->pDevice;
					inst->Internal.SKTcpLogger.pFile=inst->pFile;
					inst->Internal.SKTcpLogger.pSrc=(UDINT)&inst->Status.DataRecv;
		...
		SKTcpLogger(&inst->Internal.SKTcpLogger); 
		...
		if( inst->Internal.TCP_Open.ident != 0 ) {
			inst->Internal.TCP_Close.enable = 1;
			inst->Internal.TCP_Close.ident = inst->Internal.TCP_Open.ident;
			inst->Internal.TCP_Close.how = 0;
			TcpClose(&inst->Internal.TCP_Close);

define([
    ...
    p._pageChange = function () {
        if (this.dt.exists()) {
            var last,
                curr = this._getDataTablePageInfo().page + 1;
            if (this.internal.searchInProgress || this.widget.settings.filterConfiguration.length > 0) {
                last = this._getDataTablePageInfo().pages;
            } else {
                last = this.widget.controller.getTotalNumberOfPages();
            }
